    ...
    if (files) {
      for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
          ...
          nimModules[projectDir].push(
            createNimModule(projectDir, rootDir, dir, file),
          );
        }
      }
    }
...
async function initNimbleModules(rootDir: string) {
  ...
  for (const moduleName of nimbleModuleNames) {
    try {
      let out = cp
        .execSync(getNimbleExecPath() + " --y dump " + moduleName, {
          cwd: rootDir,
        })
        .toString();
...
export function getImports(
  ...
  for (const nimbleModule of nimbleModules) {
    if (!prefix || nimbleModule.name.startsWith(prefix)) {
      var suggestion: vscode.CompletionItem = { label: nimbleModule.name };
      suggestion.kind = vscode.CompletionItemKind.Module;
      if (nimbleModule.version) {
        suggestion.detail =
          nimbleModule.name + " [" + nimbleModule.version + "]";
      } else {
  ...
  if (nimModules[projectDir]) {
    for (const nimModule of nimModules[projectDir]) {
      if (!prefix || nimModule.name.startsWith(prefix)) {
        var suggest: vscode.CompletionItem = { label: nimModule.name };
...
export async function initImports() {
  ...
  if (isProjectMode()) {
    for (const project of getProjects()) {
      await initNimDirectories(
        vscode.Uri.parse(project.wsFolder.uri).fsPath,
        project.filePath,
      );
    }
  } else {
...
export async function addFileToImports(file: string) {
  if (isProjectMode()) {
    for (const project of getProjects()) {
      let projectDir = vscode.Uri.parse(project.wsFolder.uri).fsPath;
    ...
    if (vscode.workspace.workspaceFolders) {
      let projectDir = vscode.Uri.parse(
