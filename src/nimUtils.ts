...
export function getNimExecPath(executable: string = "nim"): string {
  ...
  if (!path) {
    vscode.window.showInformationMessage(
      `No \'${executable}\' binary could be found in PATH environment variable`,
    );
  }
  return path;
}
...
export function isWorkspaceFile(filePath: string): boolean {
  if (vscode.workspace.workspaceFolders) {
    for (const wsFolder of vscode.workspace.workspaceFolders) {
      const uri = vscode.Uri.parse(wsFolder.uri);
      if (
        uri.scheme === "file" &&
        filePath.toLowerCase().startsWith(uri.fsPath.toLowerCase())
      ) {
        return true;
      }
    }
...
export function toProjectInfo(filePath: string): ProjectFileInfo {
        ...
        if (parsedPath.length > 1) {
          for (const folder of vscode.workspace.workspaceFolders) {
            if (parsedPath[0] === folder.name) {
              return {
                wsFolder: folder,
                filePath: filePath.substr(parsedPath[0].length + 1),
              };
            }
          }
        }
  ...
  return {
    wsFolder: {
      uri: vscode.Uri.file(parsedPath.dir).toString(),
      name: "root",
    },
...
export function toLocalFile(project: ProjectFileInfo): string {
  const uri = vscode.Uri.parse(project.wsFolder.uri);
  return uri.with({
    path: uri.path + "/" + project.filePath,
  }).fsPath;
}
...
export function getDirtyFile(document: vscode.TextDocument): string {
  var dirtyFilePath = path.normalize(
    path.join(os.tmpdir(), "vscodenimdirty.nim"),
  );
    ...
    if (projectMapping instanceof Object) {
      for (const key in projectMapping) {
        if (projectMapping.hasOwnProperty(key)) {
          ...
          _projectMapping.push({
            fileRegex: new RegExp(key),
            projectPath: path,
          });
        }
      }
    ...
    if (process.platform !== "win32") {
      try {
        ...
        if (process.platform === "darwin") {
          nimPath = cp
            .execFileSync("readlink", [_pathesCache[tool]])
            .toString()
            .trim();
          if (nimPath.length > 0 && !path.isAbsolute(nimPath)) {
            nimPath = path.normalize(
              path.join(path.dirname(_pathesCache[tool]), nimPath),
            );
          }
        } else if (process.platform === "linux") {
          nimPath = cp
            .execFileSync("readlink", ["-f", _pathesCache[tool]])
            .toString()
            .trim();
        } else {
          nimPath = cp
