...
import { QuickPickItem, Uri } from "coc.nvim";
import {
  execNimSuggest,
  NimSuggestResult,
  NimSuggestType,
...
let executors: {
  [project: string]: { initialized: boolean; process?: cp.ChildProcess };
} = {};
...
function nimExec(
  ...
  return new Promise((resolve, reject) => {
    ...
    let executor = cp.spawn(getNimExecPath(), [command, ...args], {
      cwd: Uri.parse(project.wsFolder.uri).fsPath,
    });
      ...
      if (signal === "SIGKILL") {
        ...
        try {
          ...
          if (split.length === 1) {
            ...
            if (lfSplit.length > split.length) split = lfSplit;
          }
    ...
    if (!match) {
      ...
      if (msg === "template/generic instantiation from here") {
        ...
        if (isWorkspaceFile(file)) {
          ...
          ret.push({
            file: lastFile.file,
            line: parseInt(lastFile.line),
            column: parseInt(lastFile.column),
            msg: msg,
            severity,
          });
        }
...
export function check(
  filename: string,
  nimConfig: vscode.WorkspaceConfiguration,
  ...
  if (!!nimConfig["useNimsuggestCheck"]) {
    runningToolsPromises.push(
      new Promise((resolve, reject) => {
        execNimSuggest(NimSuggestType.chk, filename, 0, 0, "")
          .then((items) => {
    ...
    if (!isProjectMode()) {
      ...
      runningToolsPromises.push(
        nimExec(
          project,
          "check",
          ["--listFullPaths", project.filePath],
          true,
          parseErrors,
        ),
      );
  ...
  return Promise.all(runningToolsPromises).then((resultSets) =>
...
export async function execSelectionInTerminal(document?: vscode.TextDocument) {
  if (vscode.window.activeTextEditor) {
    const text = vscode.window.activeTextEditor.document.textDocument.getText(
      (await vscode.window.getSelectedRange("v")) ??
        fullRange(vscode.window.activeTextEditor.document.textDocument),
    );
    if (!evalTerminal) {
      ...
      evalTerminal = await vscode.window.createTerminal({
        name: "Nim Console",
      });
