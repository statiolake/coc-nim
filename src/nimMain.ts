...
import {
  addFileToImports,
  initImports,
  removeFileFromImports,
} from "./nimImports";
import * as indexer from "./nimIndexer";
import { NIM_MODE } from "./nimMode";
import {
  NimDocumentSymbolProvider,
  NimWorkspaceSymbolProvider,
} from "./nimOutline";
...
import { closeAllNimSuggestProcesses, initNimSuggest } from "./nimSuggestExec";
...
export function activate(ctx: vscode.ExtensionContext): void {
  ...
  vscode.commands.registerCommand(
    "nim.execSelectionInTerminal",
    execSelectionInTerminal,
  );

  if (
    vscode.workspace.getConfiguration("nim").get("enableNimsuggest") as boolean
  ) {
    initNimSuggest();
    ctx.subscriptions.push(
      vscode.languages.registerCompletionItemProvider(
        NIM_MODE,
        new NimCompletionItemProvider(),
        ".",
        " ",
      ),
    );
    ...
    ctx.subscriptions.push(
      vscode.languages.registerRenameProvider(
        NIM_MODE,
        new NimRenameProvider(),
      ),
    );
    ctx.subscriptions.push(
      vscode.languages.registerDocumentSymbolProvider(
        NIM_MODE,
        new NimDocumentSymbolProvider(),
      ),
    );
    ctx.subscriptions.push(
      vscode.languages.registerSignatureHelpProvider(
        NIM_MODE,
        new NimSignatureHelpProvider(),
        "(",
        ",",
      ),
    );
  ...
  ctx.subscriptions.push(
    vscode.languages.registerWorkspaceSymbolProvider(
      new NimWorkspaceSymbolProvider(),
    ),
  );
  ...
  if (
    ...
    !!vscode.workspace.getConfiguration("nim")["lintOnSave"]
  ) {
      ...
      errors.forEach((error) => {
        if (!err[error.file + error.line + error.column + error.msg]) {
          ...
          if (error.msg.indexOf("'") >= 0) {
            endColumn +=
              error.msg.lastIndexOf("'") - error.msg.indexOf("'") - 2;
          }
...
function runFile() {
  ...
  if (editor) {
    ...
    if (editor.document.isUntitled) {
      ...
      let outputDirConfig =
        vscode.workspace.getConfiguration("nim")["runOutputDirectory"];
      var outputParams = "";
      if (!!outputDirConfig) {
