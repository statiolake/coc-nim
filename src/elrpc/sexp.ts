...
export interface SExpNumber {
  kind: "number";
  n: number;
}
export interface SExpIdent {
  kind: "ident";
  ident: string;
}
...
export function parseSExp(input: string): /*SExp*/ any[] | string {
  ...
  function parseSymbol(): /*SExpIdent | SExpNumber | SExpNil*/
  string | number | null {
    let symbolStart = ptr;
    while (ptr < input.length && !(input[ptr] === " " || input[ptr] === ")")) {
      if (input[ptr] === "\\" && input[ptr + 1] === " ") ptr += 2;
      else ptr++;
    }
  ...
  function parseString(): /*SExpString*/ string {
    ...
    return hasEscapes
      ? JSON.parse(input.substring(startPos - 1, ptr + 1))
      : input.substring(startPos, ptr);
  }
  ...
  function parseListOrCon(root?: boolean): /*SExpList | SExpCons*/ any[] {
    ...
    while (ptr < input.length) {
      if (
        ...
        if (sym === ".") {
          if (items.length === 1) cons = true;
          else throw "Invalid cons cell syntax";
        }
        items.push(sym);
      } else if (input[ptr] === "(") {
        ptr++;
